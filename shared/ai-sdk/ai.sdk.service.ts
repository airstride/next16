...
import { StreamEventType as EventType } from "./types";
...
export class AISdkService {
  ...
  /**
   * Stream structured output generation (for real-time UIs)
   *
   * Returns a stream of partial objects as they're being generated by the AI.
   * Perfect for showing progress to users in real-time.
   *
   * @example
   * const UserSchema = z.object({
   *   name: z.string(),
   ...
   * });
   *
   * const stream = await aiService.streamStructuredOutput({
   *   prompt: 'Generate user profile for John Smith',
   *   schema: UserSchema,
   *   config: { provider: AIProvider.GOOGLE }
   * });
   *
   ...
   */
  async streamStructuredOutput<T = any>(params: {
    prompt?: string;
    messages?: ConversationMessage[];
    schema: z.ZodType<T>;
    config?: GenerateStructuredConfig;
  }): Promise<GenerateStructuredStreamResult<T>> {
    const { prompt, messages, schema, config = {} } = params;
    const provider = config.provider ?? this.defaultProvider;
    const modelName = config.model ?? this.defaultModel;
   ...
   *
   * This is a powerful function that combines:
   * 1. Web search for gathering information
   * 2. Streaming structured extraction
   * 3. Progress events for UX updates
   *
   * Perfect for long-running research tasks where you want to show users what's happening.
   *
   * @example
   ...
   * });
   *
   * const stream = await aiService.streamStructuredOutputWithWebSearch({
   *   prompt: 'Research Acme Corporation',
   ...
   * }
   */
  async streamStructuredOutputWithWebSearch<T = any>(params: {
    prompt?: string;
    messages?: ConversationMessage[];
    schema: z.ZodType<T>;
    config?: WebSearchConfig & StreamStructuredConfig;
  }): Promise<
    GenerateStructuredStreamResult<T> & {
      sources?: any;
  ...
  > {
    ...
    const emitEvent = (event: StreamEvent<T>) => {
      if (eventStreamController) {
        eventStreamController.enqueue(event);
      }
    };
    ...
    emitEvent({
      ...
      message: config.progressMessages?.search || "Starting web search...",
      step: "search",
      progress: 0,
    });
    ...
    try {
      ...
      searchResult = await this.generateTextWithWebSearch({
        prompt,
        messages,
        config,
      });
      ...
      emitEvent({
        ...
        message: "Web search completed. Extracting structured data...",
        step: "extract",
        progress: 40,
        metadata: {
    ...
    // Step 2: Stream structured extraction
    const extractionPrompt = `Based on this research, extract the structured data:\n\n${searchResult.text}`;

    const structuredStream = await this.streamStructuredOutput({
      prompt: extractionPrompt,
      schema,
      config,
    });

    // Create a new stream that includes progress events
    const enhancedPartialStream = new ReadableStream<Partial<T>>({
      async start(controller) {
        try {
          let lastProgress = 40;
          const reader = structuredStream.partialObjectStream.getReader();
    ...
    // Enhanced object promise with completion event
    ...
    return {
      partialObjectStream: enhancedPartialStream,
      objectPromise: enhancedObjectPromise,
      usage: structuredStream.usage,
      eventStream,
      sources: searchResult.sources,
      searchTextPromise: Promise.resolve(searchResult.text),
    };
  }
 ...
 *
 * Returns a stream of partial objects as they're being generated by the AI.
 * Perfect for showing real-time progress to users.
 *
 * @example
 * const CompanySchema = z.object({
 *   name: z.string(),
 ...
 *   prompt: 'Extract company info from: Acme Corp is a tech company with 500 employees',
 *   schema: CompanySchema,
 *   config: { provider: AIProvider.GOOGLE }
 * });
 *
 ...
 * }
 */
export async function streamStructuredOutput<T = any>(params: {
  prompt?: string;
  messages?: ConversationMessage[];
  schema: z.ZodType<T>;
  config?: GenerateStructuredConfig;
}): Promise<GenerateStructuredStreamResult<T>> {
  return aiSdkService.streamStructuredOutput(params);
}
 ...
 * }
 */
export async function streamStructuredOutputWithWebSearch<T = any>(params: {
  prompt?: string;
  messages?: ConversationMessage[];
  schema: z.ZodType<T>;
  config?: WebSearchConfig & StreamStructuredConfig;
...
> {
  return aiSdkService.streamStructuredOutputWithWebSearch(params);
}
