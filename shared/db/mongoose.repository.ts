 ...
 * MongooseRepository
 *
 * MongoDB/Mongoose-specific implementation of the IRepository interface.
 * This class is tightly coupled to Mongoose and provides MongoDB-specific
 * operations like aggregation pipelines, population, etc.
 *
 * Type Parameters:
 *   TEntity - The entity document type (must extend IEntity with ObjectId and Mongoose Document).
 *
 * Usage:
 *   const repo = new MongooseRepository<UserDocument>("User");
 *   const users = await repo.find({ name: "John" });
 *
 * Key Methods:
...
export class MongooseRepository<
  TEntity extends IEntity<Types.ObjectId> & Document
