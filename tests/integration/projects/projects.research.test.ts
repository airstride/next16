...
/**
 * Integration Test: Project Website Research
 *
 * Tests the AI-powered website research functionality for project creation.
 * Uses real AI providers to verify the complete research workflow.
 */

import { projectsService } from "@/modules/projects/application/service";
import {
  AIModel,
  AIProvider,
  TemperaturePreset,
...
describe("Project Website Research Integration Test", () => {
  ...
  describe("AI Website Research", () => {
    it(
      ...
      async () => {
        ...
        console.log(`  Products: ${result.product.features?.length || 0} features`);
        console.log(`  ICP target industries: ${result.icp.target_industries?.length || 0}`);
        ...
        expect(result.company.website).toBeDefined();
        ...
        if (result.research_notes) {
          console.log(`  Research notes: ${result.research_notes.substring(0, 100)}...`);
        }
      },
      TEST_TIMEOUT
    );
    ...
    it(
      ...
      async () => {
        ...
        console.log(`✅ Shopify research completed in ${(duration / 1000).toFixed(2)}s`);
        console.log(`  Company name: ${result.company.name}`);
        console.log(`  Industry: ${result.company.industry || "N/A"}`);
        console.log(`  Products: ${result.product.features?.length || 0} features`);
        console.log(`  Overall confidence: ${result.confidence.overall}`);
    ...
    it(
      ...
      async () => {
        ...
        try {
          ...
          console.log(`✅ Handled invalid website in ${(duration / 1000).toFixed(2)}s`);
          console.log(`  Overall confidence: ${result.confidence.overall}`);

          // Should return low confidence or minimal data
          expect(result.confidence.overall).toBeLessThan(0.5);
          console.log(`  Low confidence returned as expected: ${result.confidence.overall}`);
        } catch (error) {
          // Or it might throw an error - both behaviors are acceptable
          console.log(`✅ Error thrown for invalid website (expected behavior)`);
  ...
  describe("Create Project from Website Research", () => {
    it(
      "should create project from airstride.ai website research",
      async () => {
        const websiteUrl = "https://airstride.ai";
        const mockUserId = "test-user-123";
        const mockOrgId = "test-org-456";

        console.log("🚀 Creating project from website research...");
        console.log(`  Website: ${websiteUrl}`);
        ...
        console.log(`  Website: ${project.company.website}`);
        console.log(`  Research source: ${project.research_metadata.source}`);
        console.log(`  Research status: ${project.research_metadata.status}`);
        console.log(`  Confidence: ${project.research_metadata.confidence || "N/A"}`);

        // Validate project was created correctly
        expect(project).toBeDefined();
        expect(project.id).toBeDefined();
        expect(project.company.name).toBeDefined();
        expect(project.company.website).toBe(websiteUrl);

        // Validate research metadata
        expect(project.research_metadata).toBeDefined();
        expect(project.research_metadata.source).toBe("ai");
        expect(project.research_metadata.status).toBe("researched");
        expect(project.research_metadata.researched_at).toBeDefined();

        // Validate user ownership
        expect(project.user_id).toBe(mockUserId);
        expect(project.organization_id).toBe(mockOrgId);

        // Validate audit fields
        expect(project.created_at).toBeDefined();
        expect(project.created_by).toBe(mockUserId);

        // Log extracted data quality
        console.log("\n📊 Extracted Data Quality:");
        console.log(`  Product description: ${project.product.description ? "✓" : "✗"}`);
        console.log(`  Features: ${project.product.features?.length || 0} items`);
        console.log(`  ICP description: ${project.icp.description ? "✓" : "✗"}`);
        console.log(`  Pain points: ${project.icp.pain_points?.length || 0} items`);
        console.log(`  Brand voice tone: ${project.brand_voice.tone || "N/A"}`);
        console.log(`  Marketing assets: ${project.marketing_assets.linkedin_url ? "LinkedIn ✓" : "LinkedIn ✗"}`);

        // Verify we got meaningful data
        expect(project.product.description).toBeDefined();
        expect(project.icp.description).toBeDefined();
      },
      ...
      async () => {
        const testCases = [
          ...
          },
          {
            ...
            expectedIndustryKeywords: ["developer", "deployment", "frontend", "platform"],
          },
        ];
        ...
        for (const testCase of testCases) {
          ...
          const startTime = Date.now();
          const result = await projectsService.researchWebsite(testCase.url);
          const duration = Date.now() - startTime;

          console.log(`✅ ${testCase.name} researched in ${(duration / 1000).toFixed(2)}s`);
          console.log(`  Company: ${result.company.name}`);
          console.log(`  Industry: ${result.company.industry || "N/A"}`);
          console.log(`  Product features: ${result.product.features?.length || 0}`);
          ...
          if (hasRelevantKeyword) {
            ...
            console.log(`  ⚠️  Industry keywords not found (may still be valid)`);
          }
        }
      },
  ...
  describe("Research Quality and Confidence", () => {
    it(
      ...
      async () => {
        ...
        console.log("🔍 Testing research quality for well-known company (Stripe)...");
  ...
  describe("Growth Strategy Intelligence Extraction (ENHANCED)", () => {
    ...
    it(
      ...
      async () => {
        ...
        console.log("  Expected: Competitors, metrics, content, tech stack, team, funnel");
        ...
        if (result.competitors && result.competitors.length > 0) {
          ...
          result.competitors.forEach((comp, idx) => {
            ...
            if (comp.positioning) console.log(`       Positioning: ${comp.positioning.substring(0, 60)}...`);
            ...
            if (comp.estimated_monthly_traffic) {
              console.log(`       Traffic: ~${comp.estimated_monthly_traffic.toLocaleString()}/mo`);
            }
          });
        ...
        if (result.current_metrics) {
          ...
          if (metrics.monthly_traffic) {
            console.log(`    ✓ Monthly traffic: ${metrics.monthly_traffic.toLocaleString()}`);
          }
          if (metrics.traffic_sources && metrics.traffic_sources.length > 0) {
            console.log(`    ✓ Traffic sources: ${metrics.traffic_sources.length} channels`);
            metrics.traffic_sources.forEach(source => {
              console.log(`       - ${source.source}: ${source.percentage || "N/A"}%`);
            });
          }
          if (metrics.top_keywords && metrics.top_keywords.length > 0) {
            console.log(`    ✓ Top keywords: ${metrics.top_keywords.length} keywords`);
            console.log(`       Sample: ${metrics.top_keywords.slice(0, 3).join(", ")}`);
          }
        ...
        if (result.content_inventory) {
          ...
          if (content.publishing_frequency) {
            console.log(`    ✓ Publishing frequency: ${content.publishing_frequency}`);
          }
          if (content.content_themes && content.content_themes.length > 0) {
            console.log(`    ✓ Content themes: ${content.content_themes.join(", ")}`);
          }
          if (content.top_performing_content && content.top_performing_content.length > 0) {
            console.log(`    ✓ Top performing: ${content.top_performing_content.length} URLs`);
          }
        ...
        if (result.tech_stack) {
          ...
          if (tech.marketing_automation) {
            console.log(`    ✓ Marketing automation: ${tech.marketing_automation}`);
          }
        ...
        if (result.resources) {
          ...
          if (resources.marketing_team_size) {
            console.log(`    ✓ Marketing team: ${resources.marketing_team_size}`);
          }
          ...
          if (resources.has_in_house_design !== undefined) {
            console.log(`    ✓ In-house design: ${resources.has_in_house_design ? "Yes" : "No"}`);
          }
          if (resources.has_in_house_dev !== undefined) {
            console.log(`    ✓ In-house dev: ${resources.has_in_house_dev ? "Yes" : "No"}`);
          }
          if (resources.monthly_marketing_budget) {
            console.log(`    ✓ Marketing budget: $${resources.monthly_marketing_budget.toLocaleString()}/mo`);
          }
        ...
        if (result.conversion_funnel) {
          ...
          if (funnel.awareness_channels && funnel.awareness_channels.length > 0) {
            console.log(`    ✓ Awareness channels: ${funnel.awareness_channels.join(", ")}`);
          }
          if (funnel.consideration_assets && funnel.consideration_assets.length > 0) {
            console.log(`    ✓ Consideration assets: ${funnel.consideration_assets.join(", ")}`);
          }
          if (funnel.decision_triggers && funnel.decision_triggers.length > 0) {
            console.log(`    ✓ Decision triggers: ${funnel.decision_triggers.join(", ")}`);
          }
          if (funnel.primary_cta) {
            console.log(`    ✓ Primary CTA: "${funnel.primary_cta}"`);
          }
          if (funnel.conversion_bottleneck) {
            console.log(`    ✓ Conversion bottleneck: ${funnel.conversion_bottleneck}`);
          }
          if (funnel.avg_sales_cycle_days) {
            console.log(`    ✓ Avg sales cycle: ${funnel.avg_sales_cycle_days} days`);
          }
        ...
        console.log(`    Factual confidence: ${result.confidence.factual.toFixed(2)}`);
        console.log(`    Inferred confidence: ${result.confidence.inferred.toFixed(2)}`);
      ...
      async () => {
        ...
        if (result.competitors && result.competitors.length > 0) {
          ...
          result.competitors.forEach(comp => {
            ...
            console.log(`    - ${comp.name}${comp.website ? ` (${comp.website})` : ""}`);
          });

          // Should have strategic data (positioning, strengths, or weaknesses)
          const hasStrategicData = result.competitors.some(
            comp => comp.positioning || 
                   (comp.strengths && comp.strengths.length > 0) || 
                   (comp.weaknesses && comp.weaknesses.length > 0)
          );

          if (hasStrategicData) {
            console.log("  ✓ Competitors include strategic intelligence (positioning/strengths/weaknesses)");
          }
